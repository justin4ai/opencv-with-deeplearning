...
def onMouse(event, x, y, flags, param):
    ...
    if (breaker == 1) & (event == cv2.EVENT_LBUTTONUP):
        ...
        trans_mat = np.float32([[1, 0, -int(cv2.subtract(F_point, S_point)[0])], [0, 1, -int(cv2.subtract(F_point, S_point)[1])]])
        dst = cv2.warpAffine(image, trans_mat, (image.shape[::-1][1], image.shape[::-1][2]), cv2.INTER_LINEAR)
