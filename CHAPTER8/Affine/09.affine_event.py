...
def affine(img):
    ...
    draw_rect('image', np.copy(image), pts1)    # 입력 영상에 좌표 표시
    draw_rect('dst', dst, pts2)     # 목적 영상에 좌표 표시
...
check = -1  # 선택된 자표 사각형
pts1 = np.float32([(30, 30), (450, 30), (200, 370)])    # 입력 영상 3개 좌표
pts2 = np.float32([(30, 30), (450, 30), (200, 370)])    # 목적 영상 3개 좌표

draw_rect('image', np.copy(image), pts1)    # 입력 영상에 좌표 사각형 그리기
draw_rect('dst', np.copy(image), pts2)  # 결과 영상에 좌표 사각형 그리기
